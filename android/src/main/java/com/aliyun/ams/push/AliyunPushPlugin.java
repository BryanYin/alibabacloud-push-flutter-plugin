...
public class AliyunPushPlugin implements FlutterPlugin, MethodCallHandler {
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        ...
        } else if ("setBadgeNum".equals(methodName)) {
    ...
    private void setBadgeNum(MethodCall call, Result result) {
        Integer num = call.argument("badgeNum");
        HashMap<String, String> map = new HashMap<>();
        if (num == null) {
            map.put(CODE_KEY, CODE_PARAM_ILLEGAL);
            map.put(ERROR_MSG_KEY, "Badge number can not be empty");
            try {
                result.success(map);
            } catch (Exception e) {
                AliyunPushLog.e(TAG, Log.getStackTraceString(e));
            ...
            final CloudPushService pushService = PushServiceFactory.getCloudPushService();
            pushService.setBadgeNum(mContext, num);
            map.put(CODE_KEY, CODE_SUCCESS);
            try {
                result.success(map);
            } catch (Exception e) {
